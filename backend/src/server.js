...
// Screening map based on the Legal Metrology (Packaged Commodities) Rules, 2011
// and relevant amendments. Conditional/visual requirements are deliberately marked
// REVIEW when OCR alone cannot prove compliance.
const RULES = [
...
function evaluateText(text = '') {
  ...
  const checks = RULES.map(rule => {
    if (rule.visualOnly) {
      return {
        ruleId: rule.id, key: rule.key, title: rule.title, legalBasis: rule.legalBasis,
        status: 'REVIEW', evidence: null,
        message: 'Image/layout verification required; OCR alone cannot prove this requirement.',
        weight: 0, visualOnly: true,
      };
    }
    ...
    if (rule.negative) {
      ...
      return {
        ruleId: rule.id, key: rule.key, title: rule.title, legalBasis: rule.legalBasis,
        status: pass ? 'PASS' : 'FAIL', evidence: match ? match[0] : null,
        message: pass ? 'No misleading quantity wording detected in OCR.' : 'Potentially misleading quantity wording detected; manual verification required.',
        weight: rule.weight, conditional: Boolean(rule.conditional),
      };
    }
    ...
    return {
      ruleId: rule.id, key: rule.key, title: rule.title, legalBasis: rule.legalBasis,
      status, evidence: match ? match[0] : null,
      message: match ? 'Required declaration/pattern detected in OCR.' : rule.conditional ? 'Conditional requirement not confidently detected; determine applicability and verify manually.' : 'Declaration was not confidently detected; manual verification required.',
      weight: rule.weight, conditional: Boolean(rule.conditional),
    };
  });
...
Confidence must be an integer from 0 to 100 and should reflect legibility of the visible text, not legal compliance. If a field is not visible, leave it empty.` }];
    ...
    res.json({ provider: 'Google Gemini Vision', model: result.model, productName: parsed.productName || 'Unknown product', extractedText: text, confidence: Math.max(0, Math.min(100, Number(parsed.confidence) || 0)), fields: parsed.fields || {}, notes: Array.isArray(parsed.notes) ? parsed.notes : [], photoCount: req.files.length });
  } catch (error) {
