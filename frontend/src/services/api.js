...
function getApiUrl(path) {
  if (!API_BASE) {
    throw new Error('AI OCR backend is not configured. Set VITE_API_URL to the deployed backend URL.');
  }
...
async function request(path, options = {}) {
  ...
  if (!response.ok) {
    const message = typeof body === 'object' && body?.error ? body.error : 'The server request failed.';
...
export function analyzeInspection({ extractedText, productName, ocrConfidence, ocrProvider }) {
  return request('/api/inspections/analyze', {
    ...
    body: JSON.stringify({ extractedText, productName, ocrConfidence, ocrProvider }),
  });
}

export function persistInspection(inspection) {
  return request('/api/inspections/persist', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
